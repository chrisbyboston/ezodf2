...
`stream` is a file-like object, opened in binary mode::
    ...
    buffer = stream.read()
    doc = ezodf2.open(buffer)

**How to create a new ODF document?**

::

   doc = ezodf2.newdoc(doctype, filename)
...
::
   ...
   doc = ezodf2.opendoc(buffer)
   # or use it as template for a new document
   doc = ezodf2.newdoc(template=buffer)
...
::
   ...
   doc = ezodf2.newdoc(doctype='odt', filename='text.odt')
   # or open an existing text document
   doc = ezodf2.opendoc('text.odt')
...
Use the :func:`ezodf2.ezlist` function, creates an unnumbered list as default, use
the `style_name` parameter to use a different list-style::

   ulist = ezodf2.ezlist(['Point 1', 'Point 2', 'Point 3'])
...
::
   ...
   from ezodf2 import newdoc, opendoc, Sheet
...
examples::
   ...
   from ezodf2.timeparser import TimeParser
...
**ezodf2** has no calculation engine included, you can get/set formulas as strings,
